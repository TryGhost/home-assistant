...
def mock_ghost_data():
    ...
    return {
        ...
        "comments": 156,
        "newsletters": [
            {"id": "nl1", "name": "Weekly", "status": "active", "count": {"members": 800}},
            {"id": "nl2", "name": "Archive", "status": "archived", "count": {"members": 200}},
        ],
    }


@pytest.fixture
def mock_ghost_api(mock_ghost_data) -> Generator[AsyncMock]:
    """Mock the GhostAdminAPI."""
    with patch("custom_components.ghost.GhostAdminAPI") as mock_api_class:
        mock_api = AsyncMock()
